import type { PartyLetter } from '@/data/elections';
import { Minus, Plus } from 'lucide-react';
import { useState, useRef, useEffect } from 'react';

interface PartyListRowProps {
...
export function PartyListRow({ partyLetter, vote, locked, onIncrement, onDecrement, onSetVote }: PartyListRowProps) {
  ...
  const handleCommit = () => {
    ...
    if (!isNaN(parsed)) onSetVote(parsed);
    setEditing(false);
  };

  return (
    <div className={`flex items-center gap-3 rounded-xl bg-card px-3 py-3 shadow-sm transition-shadow hover:shadow-md border border-dashed border-border ${locked ? 'opacity-75' : ''}`}>
      <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${partyBadgeClass[partyLetter]}`}>
        📋
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium leading-tight text-foreground">
          Listi {partyLetter.toUpperCase()}
        </p>
        <p className="text-xs text-muted-foreground">Flokkurin</p>
      </div>
